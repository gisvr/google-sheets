...
describe('Spreadsheet snippets', () => {
  ...
  it('should batch update a spreadsheet', mochaAsync(async () => {
    ...
    let prem = {
      'type': 'user',
      'role': 'writer',
      'emailAddress': 'txdev6@gmail.com'
    }
    await helpers.permissions(spreadsheetId, prem)
  ...
  it.only('should batch set a spreadsheet', mochaAsync(async () => {
    const spreadsheetId = "1nIWdM1_y82s2bQdTNsj1hLwgpesAMdfHn2pS04TEzro" // await helpers.createTestSpreadsheet();
   //https://docs.google.com/spreadsheets/d/1Vy96B5ME37wwD-iLZ1abFfeZqfMuoqL-DBDSG6dETa0
    console.log("https://docs.google.com/spreadsheets/d/" + spreadsheetId)
    await helpers.populateValues(spreadsheetId);
    const result = await snippets.batchUpdate(spreadsheetId, 'New Title', 'Hello', '2Goodbye');
    const replies = result.replies;
    expect(replies.length).toBe(2);
    const findReplaceResponse = replies[1].findReplace;
    expect(findReplaceResponse.occurrencesChanged).toBe(100);
